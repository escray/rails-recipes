...
class Admin::EventRegistrationsController < AdminController
    ...
    respond_to do |format|
      ...
      format.csv do
        ...
        csv_string = CSV.generate do |csv|
          csv << %w[报名ID 票种 姓名 Email 报名时间]
          @registrations.each do |r|
            csv << [r.id, r.ticket.name, r.name,
                    t(r.status, scope: 'registration.status'),
                    r.email, r.created_at]
          end
        end
        send_data csv_string, filename: "#{@event.friendly_id}-registrations"
        + "-#{Time.now.to_s(:number)}.csv"
      end
      format.xlsx
    end
  end
  ...
  def import
    ...
    flash[:notice] = "总共导入 #{success} 笔，失败 #{failed_records.size} 笔"
    redirect_to admin_event_registrations_path(@event)
  end
